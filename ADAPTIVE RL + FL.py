...
class DatasetAnalyzer:
    ...
    def get_performance_thresholds(self, historical_rmse=None):
        if historical_rmse is not None and len(historical_rmse) > 10:
            percentiles = np.percentile(historical_rmse, [10, 25, 50, 75, 90])
            return {
                'excellent': percentiles[0],
                'good': percentiles[1],
                'average': percentiles[2],
                'poor': percentiles[3],
                'bad': percentiles[4]
            }
        else:
...
class AdaptiveFedPruningRL:
    ...
    def update_performance_thresholds(self):
        all_rmse = []
        for client_history in self.client_performance_history:
            all_rmse.extend(client_history)
        
        if len(all_rmse) > 20:
            self.performance_thresholds = self.analyzer.get_performance_thresholds(all_rmse)
        else:
            self.performance_thresholds = self.analyzer.get_performance_thresholds()
        
    def get_adaptive_state(self, model, rmse, loss, sparsity, round_num, client_id, data_size):
        with torch.no_grad():
            ...
            if self.performance_thresholds:
                rmse_percentile = 0.5
                for threshold_name, threshold_val in self.performance_thresholds.items():
            ...
            improvement = 0.0
            if len(self.client_performance_history[client_id]) > 1:
                recent_history = self.client_performance_history[client_id][-5:]
                if len(recent_history) >= 2:
                    improvement = np.clip((recent_history[0] - recent_history[-1]) / max(recent_history[0], 1e-6), -1, 1)
            
            global_improvement = 0.0
            if len(self.global_performance_history) > 1:
                recent_global = self.global_performance_history[-5:]
            ...
            if len(self.client_performance_history[client_id]) > 3:
                recent_rmse = self.client_performance_history[client_id][-3:]
    ...
    def calculate_adaptive_reward(self, prev_rmse, curr_rmse, prev_sparsity, curr_sparsity, client_id):
        ...
        rmse_reward = 0
        if self.performance_thresholds:
            if curr_rmse <= self.performance_thresholds['excellent']:
                rmse_reward = rmse_improvement * 50 + 40
            elif curr_rmse <= self.performance_thresholds['good']:
                rmse_reward = rmse_improvement * 35 + 25
            elif curr_rmse <= self.performance_thresholds['average']:
                rmse_reward = rmse_improvement * 25 + 15
            elif curr_rmse <= self.performance_thresholds['poor']:
                rmse_reward = rmse_improvement * 15 + 5
            else:
                rmse_reward = rmse_improvement * 10 - 5
        else:
            ...
            if curr_rmse <= scale * 0.5:
                ...
                rmse_reward = rmse_improvement * 10
        ...
        total_reward = rmse_reward + sparsity_reward + efficiency_bonus + stability_penalty
    ...
    def train_client(self, client_id, train_loader, val_loader, epochs=20, round_num=0):
        ...
        prev_rmse = self.client_performance_history[client_id][-1] if self.client_performance_history[client_id] else self.dataset_stats.get('std', 10.0)
        prev_sparsity = 0
        ...
        state = self.get_adaptive_state(model, rmse, avg_loss, current_sparsity, 
        ...
        next_state = self.get_adaptive_state(model, new_rmse, avg_loss, new_sparsity, 
                                           round_num, client_id, data_size)
        reward = self.calculate_adaptive_reward(prev_rmse, new_rmse, prev_sparsity, new_sparsity, client_id)
        
        self.remember_continuous(state, action_tuple, reward, next_state, False)
        if len(self.memory) > 128:
            self.replay_continuous(batch_size=64)
        
        return new_rmse, avg_loss, new_sparsity, data_size
    ...
    def federated_round(self, client_loaders, val_loaders, round_num):
        ...
        for client_id in selected_clients:
            if len(client_loaders) > client_id and len(val_loaders) > client_id:
                rmse, loss, sparsity, data_size = self.train_client(
                    client_id, 
        ...
        if len(rmses) > 0:
            ...
            self.global_performance_history.append(avg_rmse)
        ...
        return np.mean(rmses) if rmses else float('inf'), np.mean(losses) if losses else 0, np.mean(sparsities) if sparsities else 0
...
def run_adaptive_experiment():
    ...
    plt.tight_layout()
    plt.savefig('adaptive_soybean_rl_federated_results.png', dpi=300, bbox_inches='tight')
    plt.show()
